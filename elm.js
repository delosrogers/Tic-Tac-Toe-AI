...
var $author$project$Main$outerWinMap = F2(
	function (gameArr, idxes) {
		var slicedArr = A2(
			...
			idxes);
		return $author$project$Main$equal3(slicedArr);
	});
	...
	function (player, model) {
		...
		return $Chadtech$elm_bool_extra$Bool$Extra$any(
			$elm$core$Array$toList(
				A2(
					$elm$core$Array$map,
					$author$project$Main$outerWinMap(model.board),
					idx_array)));
	});
var $author$project$Main$checkWinandOutput = function (model) {
	var playerXWin = A2($author$project$Main$checkPlayerWin, $author$project$Main$PlayerX, model);
	return playerXWin ? 'Player X won' : (A2($author$project$Main$checkPlayerWin, $author$project$Main$PlayerO, model) ? 'Player O won' : '');
};
